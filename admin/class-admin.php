...
private function redirect_back_to_settings() {
    ...
    try {
        // 1. Chỉ cần verify một nonce là đủ, và nên dừng ngay nếu fail
        ...
        // 3. Validate input data chặt chẽ hơn
        $action = isset($_POST['bulk_action']) ? sanitize_text_field($_POST['bulk_action']) : '';
        if (!in_array($action, ['delete', 'reset'])) {
            wp_send_json_error([
                'message' => __('Invalid action', 'wp-content-locker')
            ]);
            return;
        }

        $ids = isset($_POST['password_ids']) ? array_map('intval', $_POST['password_ids']) : [];
        if (empty($ids)) {
            wp_send_json_error([
        ...
        // 4. Thực hiện action với error handling tốt hơn
        ...
        // 5. Response với thông tin chi tiết hơn
        if ($result !== false) {
            ...
            wp_send_json_error([
                'message' => __('Operation failed', 'wp-content-locker'),
                'action' => $action
            ]);
        }

    } catch (Exception $e) {
        // 6. Log lỗi chi tiết hơn
        error_log(sprintf(
            'Password bulk action error: %s. Action: %s, IDs: %s',
            $e->getMessage(),
            $action ?? 'unknown',
            implode(',', $ids ?? [])
        ));
        ...
        wp_send_json_error([
            'message' => __('An unexpected error occurred', 'wp-content-locker')
        ]);
    }
}
