...
try {
    ...
    $current_action = $passwords_list->current_action();
    
    if ($current_action) {
        // Verify nonce
        check_admin_referer('bulk-passwords', 'bulk_action_nonce');
        
        if (isset($_POST['passwords']) && is_array($_POST['passwords'])) {
            $ids = array_map('intval', $_POST['passwords']);
            
            if (!empty($ids)) {
                switch($current_action) {
                    case 'delete':
                        $deleted = $password_manager->delete_passwords($ids);
                        if ($deleted) {
                            $message = sprintf(
                                __('%d passwords deleted successfully.', 'wp-content-locker'),
                                count($ids)
                            );
                            add_settings_error(
                    ...
                    case 'reset':
                        $reset = $password_manager->reset_password_status($ids);
                        if ($reset) {
                            $message = sprintf(
                                __('%d passwords reset successfully.', 'wp-content-locker'),
                                count($ids)
                            );
                            add_settings_error(
                                'wcl_password_messages',
...
<div class="wrap">
    ...
    <?php else: ?>
        <!-- Password List Table -->
<form method="post" id="passwords-list-form">
    <?php 
    // Add nonce field
    wp_nonce_field('bulk-passwords', 'bulk_action_nonce'); 

    // Add current page hidden field
    ?>
    <input type="hidden" name="page" value="wcl-passwords">
...
jQuery(document).ready(function($) {
    $('#passwords-list-form').on('submit', function(e) {
        var selected = $('input[name="passwords[]"]:checked').length;
        if (selected === 0) {
            e.preventDefault();
            alert('<?php echo esc_js(__('Please select items first.', 'wp-content-locker')); ?>');
            return false;
        }
    });
});
